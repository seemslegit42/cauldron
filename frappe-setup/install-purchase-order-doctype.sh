#!/bin/bash
# Script to install the Purchase Order DocType in a Frappe application
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    --help)
      ...
      echo "  --module-name NAME      Module name (default: Buying)"
...
echo "Installing Purchase Order DocType with the following configuration:"
...
cp "$DOCTYPES_DIR/purchase_order.json" "$TEMP_DIR/doctypes/"
cp "$DOCTYPES_DIR/purchase_order_item.json" "$TEMP_DIR/doctypes/"
...
echo "Purchase Order DocType installation completed successfully!"
echo ""
echo "You can now access the Purchase Order DocType in your Frappe application."
